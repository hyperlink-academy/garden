...
export const space_data_query = `*,
  ...
  spaces_in_studios(*, studios(do_id, name, members_in_studios(member), allow_members_to_join_spaces, allow_members_to_join_spaces, allow_members_to_join_spaces, allow_members_to_join_spaces, allow_members_to_join_spaces)),
  members_in_spaces(*, identity_data(*)),
  user_space_unreads(*)
` as const;

export type SpaceData = Extract<
  Awaited<ReturnType<typeof get_space_data_by_id_route.handler>>,
  { data: { success: true } }
>["data"]["data"];
